    ...
    $(".sub_button").click(function(){
        $.post('<?php echo osc_base_url(true); ?>', { email: $("#alert_email").val(), userid: $("#alert_userId").val(), alert: $("#alert").val(), page: "ajax", action: "alerts" }, 
            function(data){
                ...
                else if(data==-1) { alert('<?php _e('Invalid email address', 'twitter_bootstrap') ; ?>'); }
                else { alert('<?php _e('There was a problem with the alert', 'twitter_bootstrap') ; ?>'); }
            }
        );
        return false;
    });
...
<div class="alert_form">
    <h3>
        <strong><?php _e('Subscribe to this search', 'twitter_bootstrap') ; ?></strong>
    </h3>
    <form action="<?php echo osc_base_url(true); ?>" method="post" name="sub_alert" id="sub_alert">
        <fieldset>
            ...
            <?php } ?>
            <button type="submit" class="sub_button" ><?php _e('Subscribe now!', 'twitter_bootstrap') ; ?></button>
        </fieldset>
    </form>
</div>